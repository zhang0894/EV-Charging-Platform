...
- [二、 充电用户端 API (Driver Client)](#二-充电用户端-api-driver-client)
  ...
  - [2.4 充电核心业务流程 (检查-启动-停止-结算)](#24-充电核心业务流程-检查-启动-停止-结算)
...
- [三、 PC 运营管理端 API (Admin Management)](#三-pc-运营管理端-api-admin-management)
  ...
  - [3.3 充电站运维管理与单站销售分析 (CRUD & 销售统计)](#33-充电站运维管理与单站销售分析-crud--销售统计)
...
- **角色划分**：
  ...
  - `admin`：PC 运营管理后台。
- **免密注册登录机制**：
  - 用户端输入 11 位手机号发起登录。若数据库中不存在该手机号，系统**自动完成新用户注册**（默认昵称：`用户+手机号后4位`），并返回 Token 与用户信息。
...
- **认证方式**：公开接口 (无需 Token)
- **业务逻辑**：根据 11 位手机号校验。若用户已存在则直接返回登录凭证；若不存在则自动注册新用户（默认昵称：`用户+手机号后4位`）。
- **请求参数**：
...
#### 1. 查询钱包余额
- **接口路径**：`GET /api/v1/user/wallet/balance`
- **认证方式**：`Bearer <user_token>`
- **成功响应 (`200 OK`)**：
  ```json
  {
...
#### 2. 用户钱包充值 (模拟支付)
- **接口路径**：`POST /api/v1/user/wallet/recharge`
- **认证方式**：`Bearer <user_token>`
- **请求参数**：
  - Request Headers:
...
- **查询参数 (Query Params)**：
  - `latitude` (必填): 用户当前纬度 (如 `31.230416`)
  - `longitude` (必填): 用户当前经度 (如 `121.473701`)
  - `radius_km` (可选, 默认 `10.0`): 搜索半径 (千米)
  - `limit` (可选, 默认 `20`): 最大返回数量
  - `keyword` (可选): 按站名或区域模糊搜索
- **后端架构原理**：服务端通过进程内 R-Tree 空间索引在微秒级筛选出候选站点 ID，再读取内存状态位图聚合可用空闲桩数后返回。
- **成功响应 (`200 OK`)**：
  ```json
  {
    "code": 0,
    ...
    "data": {
      ...
      "stations": [
        ...
        {
          "station_id": 102,
          "station_name": "金融中心地下智慧充电站",
          "address": "陆家嘴金融贸易区88号地下B2层",
          "latitude": 31.240100,
          "longitude": 121.490000,
          "distance_km": 3.40,
          "price_per_kwh": 1.80,
          "service_fee_per_kwh": 0.50,
          "overtime_fee_per_15min": 8.00,
          "total_piles": 20,
          "idle_piles": 2,
          "fast_piles_idle": 1,
          "slow_piles_idle": 1,
...
#### 1. 查询当前用户历史充电订单列表
- **接口路径**：`GET /api/v1/charging/orders`
- **认证方式**：`Bearer <user_token>`
- **查询参数 (Query Params)**：
  - `page` (可选, 默认 `1`)
  - `page_size` (可选, 默认 `10`)
...
#### 3. 新增充电站
- **接口路径**：`POST /api/v1/admin/stations`
- **认证方式**：`Bearer <admin_token>`
- **请求参数**：
  - Request Body:
    ```json
    {
      "station_name": "虹桥枢纽快充中心站",
      "address": "申昆路1500号",
      "latitude": 31.192000,
      "longitude": 121.320000,
      "price_per_kwh": 1.60,
      "service_fee_per_kwh": 0.40,
      "overtime_fee_per_15min": 6.00,
      "contact_phone": "021-66667777"
    }
    ```
- **成功响应 (`200 OK`)**：
  ```json
...
#### 4. 编辑修改充电站
- **接口路径**：`PUT /api/v1/admin/stations/{station_id}`
- **认证方式**：`Bearer <admin_token>`
- **请求参数**：
  - Request Body:
    ```json
    {
      "station_name": "东软高新科技园超级充电站(二期已扩建)",
      "price_per_kwh": 1.50,
      "service_fee_per_kwh": 0.35,
      "overtime_fee_per_15min": 5.00,
      "status": 1
...
#### 5. 删除 / 下线充电站
- **接口路径**：`DELETE /api/v1/admin/stations/{station_id}`
- **认证方式**：`Bearer <admin_token>`
- **成功响应 (`200 OK`)**：
  ```json
  {
...
#### 3. 管理员手动调账 / 余额补偿
- **接口路径**：`POST /api/v1/admin/users/{user_id}/wallet/adjust`
- **认证方式**：`Bearer <admin_token>`
- **请求参数**：
  - Request Headers:

...
namespace { const QString MapKey = QStringLiteral("YUIBZ-VBM3U-EGRVW-GPKTQ-M6LEE-4EFQJ"); }

NavigationPage::NavigationPage(QWidget *p) : QWidget(p)
{
    auto *layout = new QVBoxLayout(this); m_layout = layout; layout->setContentsMargins(20, 20, 20, 16);
    ...
    m_summary = new QLabel; m_summary->setWordWrap(true); m_summary->setObjectName("Card"); m_summary->setMargin(14); layout->addWidget(m_summary);
#ifdef NCS_HAS_WEBENGINE
...
void NavigationPage::ensureMapView()
{
    ...
    if (m_mapPlaceholder) { m_layout->replaceWidget(m_mapPlaceholder, m_map); m_mapPlaceholder->deleteLater(); m_mapPlaceholder = nullptr; }
#endif
}

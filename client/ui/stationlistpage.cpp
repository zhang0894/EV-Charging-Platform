...
{
public:
    explicit StationMapCard(std::function<void(int)> stationClicked, QWidget *parent = nullptr)
        : QWidget(parent) {
        ...
        auto *bridge = new StationMapBridge(std::move(stationClicked), m_view);
        auto *channel = new QWebChannel(m_view);
        channel->registerObject(QStringLiteral("stationBridge"), bridge);
...
#ifdef NCS_HAS_WEBENGINE
        QJsonArray points;
        for (const auto &station : stations) points.append(QJsonObject{{"id", station.id}, {"lat", station.latitude}, {"lng", station.longitude}, {"online", station.isOnline}, {"name", station.name}});
        const QString markerData = QString::fromUtf8(QJsonDocument(points).toJson(QJsonDocument::Compact));
        const QString html = QStringLiteral(R"MAP(
<!doctype html><html><head><meta charset="utf-8"><style>html,body,#map{margin:0;width:100%;height:100%;overflow:hidden}</style><script src="qrc:///qtwebchannel/qwebchannel.js"></script></head>
...
let stationBridge=null;new QWebChannel(qt.webChannelTransport,channel=>{stationBridge=channel.objects.stationBridge;});
...
function createMap(){try{
  ...
  const center=new TMap.LatLng(centerLat,centerLng);const map=new TMap.Map(document.getElementById('map'),{center:center,zoom:13,viewMode:'2D',pitch:0,scrollable:false,doubleClickZoom:false,rotationControl:false,pitchable:false});
  ...
  // 腾讯默认 MarkerStyle 会把无效/本地图标回退成红色。电站标记改为
  // DOMOverlay：在线站固定绿色、下线站固定灰色，且点击直接调用 Qt 桥接。
  class StationPin extends TMap.DOMOverlay {
    onInit(options){super.onInit(options);this.station=options.station;this.position=options.position;this.dom=null;}
    createDOM(){const el=document.createElement('div');el.title=this.station.name;el.style.cssText='position:absolute;width:26px;height:26px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:'+(this.station.online?'#148B5B':'#969E9A')+';border:2px solid white;box-shadow:0 1px 5px #50605a;cursor:pointer;box-sizing:border-box;';const dot=document.createElement('div');dot.style.cssText='width:8px;height:8px;border-radius:50%;background:white;margin:7px;';el.appendChild(dot);el.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(stationBridge)stationBridge.openStation(this.station.id);});this.dom=el;return el;}
    updateDOM(){const point=this.map.projectToContainer(this.position);this.dom.style.left=(point.getX()-13)+'px';this.dom.style.top=(point.getY()-26)+'px';}
    destroyDOM(){this.dom=null;}
  }
  data.forEach(station=>new StationPin({map:map,position:new TMap.LatLng(station.lat,station.lng),station:station}));
...
</script></body></html>)MAP").arg(latitude, 0, 'f', 7).arg(longitude, 0, 'f', 7).arg(markerData);
...
StationListPage::StationListPage(QWidget *p) : QWidget(p), m_network(new QNetworkAccessManager(this))
{
    ...
    m_map = new StationMapCard([this](int stationId) { emit stationSelected(stationId); }, this); layout->addWidget(m_map);

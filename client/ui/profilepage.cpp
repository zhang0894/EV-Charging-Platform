...
ProfilePage::ProfilePage(QWidget *parent) : QWidget(parent), m_network(new QNetworkAccessManager(this))
{
    ...
    auto *texts = new QVBoxLayout; auto *nameRow = new QHBoxLayout; m_name = new QLabel(QStringLiteral("正在加载…")); m_name->setObjectName("CardTitle"); nameRow->addWidget(m_name); m_editName = new QPushButton(QStringLiteral("修改")); m_editName->setObjectName("Ghost"); nameRow->addWidget(m_editName); nameRow->addStretch(); texts->addLayout(nameRow);
    ...
    m_tip = new QLabel; m_tip->setObjectName("Cap"); m_tip->setWordWrap(true); layout->addWidget(m_tip); layout->addStretch();
...
void ProfilePage::reload()
{
    ...
    showDefaultAvatar();
    UserService::profileAsync([this](bool ok, const UserProfile &profile, const QString &error) {
        if (!ok) { m_tip->setText(error); return; }
        m_name->setText(profile.nickname); m_phone->setText(QStringLiteral("手机号：%1").arg(maskedPhone(profile.phone))); applyBalance(profile.balance); m_tip->clear();
        if (profile.avatarUrl.isEmpty()) showDefaultAvatar(); else loadAvatar(profile.avatarUrl);
    });
}
...
void ProfilePage::loadAvatar(const QString &url)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply] {
        ...
        QPixmap pixmap; pixmap.loadFromData(bytes); const QString error = reply->errorString(); reply->deleteLater();
        ...
        showDefaultAvatar();
        // 头像加载失败时仍保留可用的默认头像；不要让 404 等历史头像
        // 地址错误覆盖个人中心的正常状态提示。
        if (!networkOk && !error.isEmpty())

...
bool uploadAvatar(const QString &filePath, QString *avatarUrl, QString *error) {
    ...
    // QFileInfo 能识别 qrc 资源的扩展名；优先使用它，避免内置 JPEG
    // 被 QMimeDatabase 当成无后缀文件而错误标记为 PNG。
    QString type = QFileInfo(filePath).suffix().toLower();
    if (type.isEmpty())
        type = QMimeDatabase().mimeTypeForFile(filePath).suffixes().value(0, QStringLiteral("png"));
    const QString encoded = QStringLiteral("data:image/%1;base64,").arg(type) + QString::fromLatin1(f.readAll().toBase64());
    QJsonObject data; if (!Api::post(QStringLiteral("/api/v1/user/avatar"), {{QStringLiteral("image_base64"), encoded}, {QStringLiteral("file_type"), type}}, &data, error)) return false;
...
void profileAsync(std::function<void(bool, const UserProfile &, const QString &)> callback)
{
    Api::getAsync(QStringLiteral("/api/v1/user/profile"), [callback](bool ok, const QJsonObject &data, const QString &error) {
        UserProfile profile;
        if (ok) { profile.phone=data.value(QStringLiteral("phone")).toString(); profile.nickname=data.value(QStringLiteral("nickname")).toString(); profile.avatarUrl=cloudAvatarUrl(data.value(QStringLiteral("avatar_url")).toString()); profile.createdAt=data.value(QStringLiteral("created_at")).toString(); profile.balance=data.value(QStringLiteral("balance")).toDouble(); }
